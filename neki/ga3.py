...
def fitness_func(ga_instance, solution, solution_idx):
    ...
    # Convert the solution to a tuple to use as a cache key
    solution_key = tuple(solution)

    # Check if the fitness is already computed for this solution
    if solution_key in fitness_cache:
        return fitness_cache[solution_key]

    # Convert the solution to a tensor for TensorFlow compatibility
    solution_tensor = tf.convert_to_tensor(solution, dtype=tf.float32)
    
    # Convert the solution to the neural network
    ...
    # Compute the number of layers and total neurons as tensors
    num_layers = solution[len(BEG_PARAMS)]
    total_neurons = solution[len(BEG_PARAMS) + 1:MAX_LAYERS + len(BEG_PARAMS) + 1].sum()

    # Free up memory
    del solution_nn


    # Calculate the penalty and fitness score
    ...
    fitness_cache[solution_key] = fitness_score
...
def structured_mutation(individual):
    ...
    if np.random.rand() < mutation_probability:
        ...
        while batch_size_old == batch_size:
            batch_size = float(np.random.choice(([16, 32, 64, 128])))
