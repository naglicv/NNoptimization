...
penalty_mult_list = [5, 2, 1, 0.5, 0.1, 0.05, 0.01, 0]  # Penalty multiplier for the complexity of the network
...
def generatePopulation(sol_per_pop):
    ...
    for _ in range(sol_per_pop):
        ...
        solution = np.concatenate((
                ...
                hidden_layer_sizes.astype(np.float16), 
                activations.astype(np.float16), 
                dropout_rates.astype(np.float16), 
                batch_norms.astype(np.float16), 
                [activation_output] 
            ))
...
def fitness_func(ga_instance, solution, solution_idx):
    ...
    # Train the neural network and get validation loss
    validation_loss = train_and_get_val_loss(
                        solution_nn.model, X_train, y_train, X_val, y_val,
                        epochs=int(solution_nn.epochs), 
                        batch_size=int(solution_nn.batch_size), 
                        early_stopping=solution_nn.early_stopping)
    ...
    del solution_nn  # Delete the neural network object after evaluation
    tf.keras.backend.clear_session()  # Clear the Keras backend session to free up memory
...
def structured_crossover(parent1, parent2):
    ...
    if cross_option == 1:
        ...
        offspring1 = np.concatenate((
            ...
            new_hidden_layer_sizes1.astype(np.float16), 
            new_activations1.astype(np.float16), 
            new_dropout_rates1.astype(np.float16), 
            new_batch_norms1.astype(np.float16), 
            [parent1[-1]]
        ))
        
        offspring2 = np.concatenate((
            [new_learning_rate2, new_batch_size2, new_epochs2, new_patience2, new_num_layers2],
            new_hidden_layer_sizes2.astype(np.float16), 
            new_activations2.astype(np.float16), 
            new_dropout_rates2.astype(np.float16), 
            new_batch_norms2.astype(np.float16), 
            [parent2[-1]]
        ))
    ...
    # OPTION 2: beginning of parent 1 + "connection layer" + middle of parent 2 + "connection layer" + end of parent 1
        ...
        for i in range(2):
            ...
            offspring = np.concatenate((
                ...
                new_hidden_layer_sizes.astype(np.float16), 
                new_activations.astype(np.float16), 
                new_dropout_rates.astype(np.float16), 
                new_batch_norms.astype(np.float16), 
...
if __name__ == '__main__':
    ...
    if dataset == 'iris':
        ...
        # Split the dataset into 70% training and 30% for validation + test
        ...
        # Preprocess the data
        X_train_full = X_train_full.reshape(60000, 784).astype("float16") / 255
        X_test = X_test.reshape(10000, 784).astype("float16") / 255

        y_train_full = to_categorical(y_train_full, num_classes=10)
        y_test = to_categorical(y_test, num_classes=10)

        # Split the training data into 70% training and 30% for validation + test
    ...
    for i, penalty_mult in enumerate(penalty_mult_list):
        ...
        # Evaluate nn2 on the test dataset
